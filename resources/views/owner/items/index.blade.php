...
@section('breadcrumbs')
    <a href="{{ route('owner.dashboard') }}">Dashboard</a> › <span style="color:#0f2044;">Inventory</span>
@endsection
...
@section('content')
    <div class="page-header">
        ...
        <a href="{{ route('owner.items.create') }}" class="btn-add">+ Add New Item</a>
    </div>
    ...
    <form method="GET" action="{{ route('owner.items.index') }}" id="filterForm">
        <div class="filter-bar">
            ...
            <a href="{{ route('owner.items.export', request()->query()) }}" class="filter-btn">
                    ...
                    <tr class="{{ $isExpired || $isDamaged ? 'expired-row' : '' }}">
                        ...
                        <td>
                            <div class="actions">
                                ...
                                <form action="{{ route('owner.items.destroy', $item) }}" method="POST" style="display:inline">
                                    @csrf @method('DELETE')
                                ...
                                @if($isDamaged && !$hasPendingReturn)
                                    <form action="{{ route('owner.items.unmarkDamaged', $item) }}" method="POST" style="display:inline">
                                        @csrf @method('PATCH')
                    ...
                    <tr>
                        <td colspan="8" class="empty-state">
                            @if($keyword || $status || $category)
                                ...
                                No items match your search. <a href="{{ route('owner.items.index') }}">Clear filters</a>
                            @else
                                <div style="font-size:2rem;margin-bottom:12px">📦</div>
                                No items in inventory yet. <a href="{{ route('owner.items.create') }}">Add your first item</a>
                            @endif
                        </td>
                    </tr>
                @endforelse
