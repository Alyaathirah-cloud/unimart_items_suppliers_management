...
@section('breadcrumbs')
    <a href="{{ route('staff.dashboard') }}">Dashboard</a> › <span style="color:#0f2044;">Staff</span>
@endsection
...
@section('content')
    <div class="page-header">
        ...
        <div>
            <a href="{{ route('staff.staff.create') }}" class="btn-primary">+ Add Staff</a>
        </div>
    </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <div style="display: flex; gap: 8px;">
                                <form action="{{ route('staff.staff.approve', $user->id) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="btn-primary" style="background: #27ae60; padding: 6px 12px; font-size: 0.85rem;">Approve</button>
                                </form>
                                <form action="{{ route('staff.staff.reject', $user->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to reject this request?');">
                                    @csrf
                                    @method('PATCH')
            ...
            <tbody>
                @forelse($activeStaff as $user)
                    <tr>
                        ...
                        <td>
                            <div style="display: flex; gap: 8px;">
                                <a href="{{ route('staff.staff.edit', $user->id) }}" class="btn-primary" style="padding: 6px 12px; font-size: 0.85rem;">Edit</a>
                                
                                @if($user->status !== 'rejected')
                                    <form action="{{ route('staff.staff.toggle-status', $user->id) }}" method="POST">
                                        @csrf
                                        @method('PATCH')
                                ...
                                <a href="{{ route('staff.staff.reset-password', $user->id) }}" class="btn-primary" style="background: #34495e; padding: 6px 12px; font-size: 0.85rem;">Reset Password</a>

                                <form action="{{ route('staff.staff.destroy', $user->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this staff account?');">
                                    @csrf
                                    @method('DELETE')
