...
@section('breadcrumbs')
    <a href="{{ route('staff.rr.index') }}">Return Requests</a> › <span style="color:#0f2044;">{{ $returnRequest->return_number }}</span>
@endsection
...
@section('content')
    <div class="content-center">
                    ...
                    <div class="field">
                        ...
                        <div class="field-value">
                            @if($returnRequest->invoice)
                                <a href="{{ route('staff.invoice.show', $returnRequest->invoice) }}" style="color:#4a90d9;text-decoration:none;font-weight:700;">{{ $returnRequest->invoice->invoice_number }}</a>
                            @else
                    ...
                    <div class="field">
                        ...
                        <div class="field-value">
                            @if($returnRequest->purchaseOrder)
                                <a href="{{ route('staff.po.show', $returnRequest->purchaseOrder) }}" style="color:#4a90d9;text-decoration:none;font-weight:700;">#{{ $returnRequest->purchaseOrder->po_number }}</a>
                            @else
                                —
                            @endif
                        </div>
                                ...
                                @foreach($returnRequest->lines as $line)
                                    <tr style="border-bottom:1px solid #f4f6fb;">
                                        ...
                                        <td style="padding:12px 14px;">
                                            @if($returnRequest->invoice)
                                                <a href="{{ route('staff.invoice.show', $returnRequest->invoice) }}" style="color:#4a90d9;text-decoration:none;">{{ $returnRequest->invoice->invoice_number }}</a>
                                            @else
                                                {{ $returnRequest->invoice_number ?? '—' }}
                                            @endif
                                        </td>
        ...
        <div style="margin-top: 24px;">
            <a href="{{ route('staff.rr.index') }}" class="btn-secondary">← Back to Return Requests</a>
        </div>
    </div>
@endsection
