...
@section('topbar')
    <div style="font-size:0.85rem;color:#7a8fa8;">
        <a href="{{ route('staff.po.index') }}" style="color:#4a90d9;text-decoration:none;font-weight:600;">Purchase Orders</a>
...
<div class="content-center">
    ...
    <div class="page-header">
        ...
        <a href="{{ route('staff.po.index') }}" class="btn-back">
            ← Back to Orders
        </a>
    </div>
    ...
    <div class="form-card">
        <form method="POST" action="{{ route('staff.po.store') }}" id="poForm">
            @csrf
            ...
            <div id="creditNotePanel" style="display:none;border:2px solid #27ae60;border-radius:10px;margin-bottom:24px;overflow:hidden;">
                <div style="background:#e8f8f0;padding:12px 18px;display:flex;align-items:center;gap:10px;">
                    ...
                    <span style="font-size:.9rem;font-weight:700;color:#1d8348;">Available Credit Note</span>
                </div>
            ...
            {{-- Hidden fields for credit application --}}
            <input type="hidden" name="apply_credit_note_id" id="apply_credit_note_id" value="">
            <input type="hidden" name="credit_amount_applied" id="credit_amount_applied" value="0">
...
<script>
    ...
    function calculateTotal() {
        ...
        const creditApplied = parseFloat(document.getElementById('credit_amount_applied').value) || 0;
    ...
    async function loadCreditNotes(supplierId) {
        ...
        // Reset any previously applied credit
        appliedCreditNoteId = null;
        appliedCreditAmount = 0;
        document.getElementById('apply_credit_note_id').value = '';
        document.getElementById('credit_amount_applied').value = 0;
        ...
        try {
            ...
            const cn = data.creditNotes[0]; // apply the first available CN
            ...
            content.innerHTML = `
                <div style="display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap;">
                    <div>
                        ...
                        <div style="font-size:.78rem;color:#5a6a85;margin-top:2px;">Balance: <strong>RM ${balance.toFixed(2)}</strong></div>
                    </div>
                    <div style="display:flex;gap:8px;">
                        <button type="button" id="applyCnYes" onclick="applyCreditNote('${cn.credit_note_id}', ${cn.id}, ${balance})"
                            style="background:#27ae60;color:#fff;border:none;border-radius:7px;padding:8px 20px;font-size:.84rem;font-weight:700;cursor:pointer;">Yes — Apply</button>
                        <button type="button" onclick="removeCreditNote()"
                            style="background:#fff;color:#c0392b;border:1px solid #f5b7b1;border-radius:7px;padding:8px 18px;font-size:.84rem;font-weight:600;cursor:pointer;">No — Skip</button>
                    </div>
                </div>
                <p style="font-size:.75rem;color:#7a8fa8;margin-top:10px;">
                    Apply to this PO? Credit will be deducted from the final payable amount.
                </p>
            `;
            panel.style.display = 'block';
        } catch(e) {
    ...
    function applyCreditNote(cnId, cnDbId, balance) {
        appliedCreditNoteId = cnDbId;
        appliedCreditAmount = balance;
        document.getElementById('apply_credit_note_id').value = cnDbId;
        document.getElementById('credit_amount_applied').value = balance;
        document.getElementById('summary_credit_applied').textContent = '- RM ' + balance.toFixed(2);
        document.getElementById('credit_row').style.display = 'flex';
        document.getElementById('creditNotePanel').style.display = 'none';
        calculateTotal();
    }
